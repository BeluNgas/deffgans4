...
const cekWaktuTG = (client, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            client.sendMessage(_dir[position].id, `*Waktu Mu Sudah habis*\n\n*Jawaban :* ${_dir[position].jawaban}\n\nMau Main Lagi?`, MessageType.text)
...
const cekWaktuSA = (client, _dir) => {
    setInterval(() => {
        ...
        if (position !== null) {
            client.sendMessage(_dir[position].id, `GAME SIAPAKAH AKU\n\n*Waktu Mu Sudah habis*\n\n*Jawaban :* ${_dir[position].jawaban}\n\nMau Main Lagi?`, MessageType.text)
            _dir.splice(position, 1)
        }
    }, 1000)
}
